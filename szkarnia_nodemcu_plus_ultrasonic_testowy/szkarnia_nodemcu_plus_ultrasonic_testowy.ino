...
WiFiClient esp_test1;
PubSubClient client(esp_test1);
...
const char* tempTopic = "szklarnia_test/temp";
const char* humTopic = "szklarnia_test/hum";
...
void callback(String topic, byte* message, unsigned int length) {
  ...
  // If a message is received on the topic szklarnia_test/podlewanie, you check if the message is either on or off. Turns the lamp GPIO according to the message
  if(topic=="szklarnia_test/cyrkulacja1"){
 ...
 // If a message is received on the topic szklarnia_test/lamp2, you check if the message is either on or off. Turns the lamp GPIO according to the message
  if(topic=="szklarnia_test/lamp2"){
  ...
  Serial.println();
   ...
   // If a message is received on the topic szklarnia_test/lamp2, you check if the message is either on or off. Turns the lamp GPIO according to the message
  if(topic=="szklarnia_test/cyrkulacja"){
...
void setup() {
  ...
  client.setCallback(callback);
    ...
    if (client.connect("ESP8266Client", mqttUser, mqttPassword)) {
      ...
      client.subscribe("szklarnia_test/cyrkulacja1"); // Subscribe to a specific MQTT topic
    } else {
...
void loop() {
  ...
  if(!client.loop())
    client.connect("esp_szklarnia_test");
...
// konieczna obrobka sygnalu https://www.hobbytronics.co.uk/arduino-float-vars   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  client.publish("szklarnia_test/temp1", temperature1);   // 
  //convert now to string now_string
  static char now_string[7];
  dtostrf(now, 6, 0, now_string);

     client.publish("szklarnia_test/milis", now_string);   //
...
//publish distance
  ...
  //const char* distTopic;
    // distTopic = "szklarnia_test/dist";  // to jest nie potrzebne
  //client.publish(distTopic, distanceStr);      // to nie dziala
  //client.publish("szklarnia_test/distance", String(distance));   // to nie dziala
client.publish("szklarnia_test/distance", String(distance).c_str());
  delay(1000);
